...
function typeLoop(){
  ...
  if(forward){
    ...
    ci--;
    if(ci===0){
      forward=true; ti=(ti+1)%texts.length;
    }
  }
...
window.addEventListener('load', ()=>{
  ...
  // fade in
  setTimeout(()=>{ typedEl.parentElement.style.transition='opacity .9s ease'; typedEl.parentElement.style.opacity=1; }, 200);
...
setInterval(()=>{ if(Math.random()<0.55) spawnShooting(); }, 700);
...
cta.addEventListener('click', (e)=>{
  ...
  for(let i=0;i<8;i++){
    ...
    dot.style.position='fixed'; dot.style.width='8px'; dot.style.height='8px'; dot.style.borderRadius='99px';
    dot.style.left = (e.clientX)+'px'; dot.style.top=(e.clientY)+'px'; dot.style.zIndex=9999; dot.style.pointerEvents='none';
